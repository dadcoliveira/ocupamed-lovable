  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-all"
          >
            + Nova Solicitação
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {/* Chips de status */}
        <div className="flex gap-2 overflow-x-auto mb-4 pb-1 flex-nowrap">
          <button
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="Buscar por colaborador, empresa, ID, CPF..."
              className="w-full text-sm border border-gray-200 rounded-lg pl-9 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white"
            />
            {filtros.busca && (
              <button
          ...
          {filtrosAtivos > 0 && (
            <button
              ...
              className="px-4 py-2.5 text-sm text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-all"
            >
              Limpar
            </button>
          )}
              ...
              <div className="flex flex-col gap-1.5">
                {(
                  ...
                  <button
                    ...
                    className={`text-left text-sm px-3 py-1.5 rounded-lg border transition-all flex justify-between items-center ${
                      ordenarPor === campo
                  ...
                  >
                    {label}
                    {ordenarPor === campo && (
                      <span className="text-blue-500">{ordemAsc ? "↑" : "↓"}</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  >
                    {filtros.prazoVencido && <span className="text-white text-xs">✓</span>}
                  </span>
                  ⏰ Prazo vencido
                </button>
                <button
                ...
                >
                  ...
                  >
                    {filtros.docsPendentes && <span className="text-white text-xs">✓</span>}
                  </span>
                  📄 Docs pendentes
                </button>
              </div>
              ...
              {filtrosAtivos > 0 && (
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {filtros.status && (
                      <span
                        className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${statusColors[filtros.status]}`}
                      >
                    ...
                    {filtros.prioridade && (
                      <span
                        className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${prioridadeColors[filtros.prioridade]}`}
                      >
                        {filtros.prioridade}
                        <button
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th
                      className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase cursor-pointer hover:text-gray-800 select-none hidden md:table-cell"
                    ...
                    <th
                      className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase cursor-pointer hover:text-gray-800 select-none hidden sm:table-cell"
                ...
                <tbody>
                  {solicitacoesFiltradas.map((sol, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 text-xs text-gray-400 font-mono">{sol.id}</td>
                        <td className="px-4 py-3">
                          <p className="font-medium text-gray-800 leading-tight">
                            {sol.colaborador}
                          </p>
                          <p className="text-xs text-gray-400">{sol.cargo}</p>
                        </td>
                        <td className="px-4 py-3 hidden md:table-cell">
                          <p className="text-gray-700 leading-tight">{sol.empresa}</p>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusColors[sol.status]}`}
                          >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${prioridadeColors[sol.prioridade]}`}
                          >
                            {sol.prioridade}
                          </span>
                        </td>
                        <td className="px-4 py-3 hidden sm:table-cell">
                          <span
                            className={`text-xs font-medium ${vencido ? "text-red-600" : "text-gray-600"}`}
                          >
                            {vencido && "⏰ "}
